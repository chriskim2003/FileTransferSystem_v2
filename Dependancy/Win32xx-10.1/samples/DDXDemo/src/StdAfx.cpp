/* (20-Oct-2024) [Tab/Indent: 8/8][Line/Box: 80/74]               (StdAfx.cpp) *
********************************************************************************

    Contents Description: Standard declarations implementation File. This file
    is used for precompiled hearders. Precompiled headers allow programs to be
    recompiled more quickly.

    To use precompiled headers, do the following:
    1) Add the set of rarely changed headers to stdafx.h
    2) Include stdafx.h in each cpp file. It must be included first.
    3) Add stdafx.cpp to your project.
    4) Turn on precompiled hearders in the project

*=============================================================================*/

#include "stdafx.h"
